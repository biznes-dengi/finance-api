...
public class JwtService {
    ...
    @Value("${spring.security.jwt.expiration-time:1}")
    ...
    private String createToken(final Map<String, Object> claims, final String subject) {
        return Jwts.builder()
                ...
                .setExpiration(new Date(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(expirationTime)))
                .signWith(this.secretKey)
                .compact();
    }

    public Boolean validateToken(final String token, final String username) {
        return extractUsername(token).equals(username) && Boolean.FALSE.equals(isTokenExpired(token));
    }
